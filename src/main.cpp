...
void debug() {
    if (running_count % 5 == 0)
        cout << "\n  =====  running: " << running_count << "  =====  " << endl
             ...
             << "curr_average = " << curr_average <<endl;
    running_count++;
}
...
void ProcessFrame() {
    ...
    if (timer.task == TASK_STOP) {
        ...
        if ((timer.next_color == green or timer.next_color == red or timer.next_color == orange or timer.next_color == blue or timer.next_color == yellow) and timer.stage == 0)
